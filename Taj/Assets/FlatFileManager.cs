...
namespace Taj.Assets
{
    public class FlatFileManager : IAssetManager
    {
        ...
        public void PutAsset(byte[] data, Messages.AssetType type, int ID, uint CRC = 0)
        {
        ...
        public byte[] GetAsset(Messages.AssetType type, int ID, uint CRC = 0)
        {
            return File.ReadAllBytes(CreateMuddyFilename(type, ID, CRC));
        }

        string CreateMuddyFilename(Messages.AssetType type, int ID, uint CRC = 0)
        {
