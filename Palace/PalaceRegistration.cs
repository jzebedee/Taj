...
namespace Palace
{
    public class PalaceRegistration
    {
        #region Const
        ...
        public struct RegistrationCode
        {
            ...
            internal RegistrationCode(uint seed)
            {
                ...
                Counter = ComputeLicenseCounter(seed, CRC);
            }

            public readonly uint CRC, Counter;
        }
        ...
        {
            seed = BitConverter.ToUInt32(BitConverter.GetBytes(seed).Reverse().ToArray(), 0);

            uint crc = CRC_MAGIC;

            for (int i = 0; i < 4; i++)
            {
                uint currentByte = seed & 0xff;
                var crcLeft = crc << 1;
                var crcAnd = crc & 0x80000000;
                crc = (crcLeft | (crcAnd == 0x0 ? 0x0u : 0x1u)) ^ CRCMASK[currentByte];
                seed = seed >> 8;
            }
        ...
        public static uint ComputeLicenseCounter(uint seed, uint crc)
        {
