...
namespace MiscUtil.IO
{
    ...
    public class EndianBinaryReader : IDisposable
    {
        ...
        public async Task<int> ReadAsync(char[] data, int index, int count)
        {
            CheckDisposed();
            if (buffer == null)
            {
            ...
            while (read < count)
            {
                ...
                int bytesRead = await TryReadInternalAsync(byteBuffer, amountToRead);
                if (bytesRead == 0)
                {
                    return read;
                }
        ...
        public async Task<int> ReadAsync(byte[] buffer, int index, int count)
        {
            ...
            while (count > 0)
            {
                int block = await stream.ReadAsync(buffer, index, count);
                if (block == 0)
                {
                    return read;
                }
        ...
        public async Task<byte[]> ReadBytesAsync(int count)
        {
            ...
            while (index < count)
            {
                int read = await stream.ReadAsync(ret, index, count - index);
                // Stream has finished half way through. That's fine, return what we've got.
                if (read == 0)
                {
                    byte[] copy = new byte[index];
        ...
        public async Task<byte[]> ReadBytesOrThrowAsync(int count)
        {
            byte[] ret = new byte[count];
            await ReadInternalAsync(ret, count);
            return ret;
        }
        ...
        async Task ReadInternalAsync(byte[] data, int size)
        {
            CheckDisposed();
            int index = 0;
            while (index < size)
            {
                int read = await stream.ReadAsync(data, index, size - index);
                if (read == 0)
                {
                    throw new EndOfStreamException
                        (String.Format("End of stream reached with {0} byte{1} left to read.", size - index,
        ...
        async Task<int> TryReadInternalAsync(byte[] data, int size)
        {
            CheckDisposed();
            int index = 0;
            while (index < size)
